...
`https://ntfy.knightwatcher.com`. Nothing here needs an account with any
company — the server is ours. Each phone needs about three minutes:

1. Install **ntfy** from the Play Store (by Philipp Heckel, free).
2. Open it → ⚙ Settings → **Default server** → `https://ntfy.knightwatcher.com`.
3. Still in Settings → **Manage users** → add user:
   - Username: `family`
   - Password: *(in `~/.config/knightwatcher/ntfy.env` on the dev machine,
     line `NTFY_FAMILY_PASSWORD=`)*
...
6. Test: from the dev machine,
   `curl -u "pi:$NTFY_PI_PASSWORD" -H "Priority: 5" -d "test" https://ntfy.knightwatcher.com/alerts`
